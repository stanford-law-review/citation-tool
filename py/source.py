...
def postprocess(text):
    ...
    text = re.sub(
        r'[ \.\/\\:"_]+', " ", text
    )  # Replace problematic punctuation with spaces
    text = text.strip(" ")  # Remove leading and trailing spaces
    text = titlecase.titlecase(text)  # Apply title case formatting
    text = re.sub(
        r"\b(Infra|Supra)(?:\s+\w+)*\s+(Ii|Iii|Iv|v)\b",
        lambda match: f"{match.group(0).rsplit(' ', 1)[0]} {match.group(2).upper()}",
        text,
    )  # Capitalize Roman numerals I-V
    text = re.sub(
        r"(\b[A-Z]\b(?:\s+\b[A-Z]\b)+)",
        lambda match: match.group(0).replace(" ", ""),
        text,
    )  # Join single-letter capitalized words
    return text
...
def unprocessed_name(text):
    ...
    if (
        " v " in initial_segment
        or " v. " in initial_segment
        or initial_segment.startswith("in re")
    ):
...
class Source:
    ...
    def __init__(self, text):
        ...
        )  # Extract and postprocess the name
        self.fullcite = text  # Store the full citation
        self.tokens = [
            preprocess(token.text.lower())
            for token in nlp(text)
            if not token.is_punct and not token.is_stop
        ]  # Tokenize citation text
    ...
    for base_name, similar_sources in sources_by_name.items():
        ...
        for source in similar_sources:
            ...
            for token in source.tokens:
                unique = all(
                    token not in other.tokens
                    for other in similar_sources
            ...
            new_source_name = (
                postprocess(f"{base_name} {unique_token}")
                if unique_token
                else base_name
            )
            ...
            source.warnings.append(
                "This source may be a duplicate."
            ) if not unique_token else None
    ...
    for citation in citations:
        ...
        # Handle "supra" references
            ...
            for candidate in citations:
                ...
                if reference.isdigit() and int(reference) == int(
                    candidate.footnote_num
                ):
            ...
            if not matched:
                citation.warnings.append(
                    "Couldn’t find original source for this supra citation."
                )
        ...
        # Handle "lookback 5" citations (reference to recent citations)
            ...
            for candidate in citations:
                if (
                    candidate.source
                    and int(citation.footnote_num) - int(candidate.footnote_num) <= 5
                ):
            ...
            if not matched:
                citation.warnings.append(
                    'Identified this as a potential "look five footnotes back" citation but couldn\'t identify the original source.'
                )
