...
def extract_citations(input_file, ai_config_file, num_acknowledgment_footnotes, enable_markup, output_folder):
    """
    ...
    if not os.path.isfile(ai_config_file):
        raise FileNotFoundError(f"AI configuration file '{ai_config_file}' does not exist.")
    ...
    try:
        ...
        sources = [citation.source for citation in citations if citation.type is CitationType.LONG_FORM]
