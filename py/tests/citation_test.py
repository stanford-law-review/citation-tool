...
from citation import classify_cite, CitationType
...
class TestClassifyCite(unittest.TestCase):
    ...
    def setUp(self):
        ...
        with open(os.path.join(self.test_data_dir, 'citation_test_input.txt'), 'r') as f:
            self.test_cases = f.readlines()
        
        with open(os.path.join(self.test_data_dir, 'citation_test_output_type.txt'), 'r') as f:
            self.expected_types = [CitationType[line.strip()] for line in f.readlines()]
        
        with open(os.path.join(self.test_data_dir, 'citation_test_output_shortcite.txt'), 'r') as f:
            self.expected_shortcites = [line.strip() for line in f.readlines()]
        
        with open(os.path.join(self.test_data_dir, 'citation_test_output_reference.txt'), 'r') as f:
    ...
    def test_classify_cite(self):
        ...
        for i, text in enumerate(self.test_cases):
            ...
            expected_shortcite = self.expected_shortcites[i] if self.expected_shortcites[i] != 'None' else None
            expected_reference = self.expected_references[i] if self.expected_references[i] != 'None' else None
            ...
            if citation_type == CitationType.SUPRA:
                if 'reference' not in lookback:
                    lookback_errors.append(f"Missing 'reference' in lookback for SUPRA: {text}")
                if 'shortcite' not in lookback:
                    lookback_errors.append(f"Missing 'shortcite' in lookback for SUPRA: {text}")
            elif citation_type == CitationType.LOOKBACK_5:
                if 'shortcite' not in lookback:
                    lookback_errors.append(f"Missing 'shortcite' in lookback for LOOKBACK_5: {text}")

            # Check lookback values
            if expected_shortcite and lookback.get('shortcite') != expected_shortcite:
                lookback_errors.append(f"Expected shortcite '{expected_shortcite}' but got '{lookback.get('shortcite')}' for: {text}")
            if expected_reference and lookback.get('reference') != expected_reference:
                lookback_errors.append(f"Expected reference '{expected_reference}' but got '{lookback.get('reference')}' for: {text}")

        if errors or lookback_errors:
            error_messages = []
            for (expected, actual), citations in errors.items():
                error_messages.append(f"Expected {expected} but got {actual} for the following citations:\n" + "\n".join(citations))
            if lookback_errors:
                error_messages.append("Lookback errors:\n" + "\n".join(lookback_errors))
            self.fail("\n\n".join(error_messages))

    def update_output(self):
        with open(os.path.join(self.test_data_dir, 'citation_test_output_type.txt'), 'w') as f_type, \
             open(os.path.join(self.test_data_dir, 'citation_test_output_shortcite.txt'), 'w') as f_shortcite, \
             open(os.path.join(self.test_data_dir, 'citation_test_output_reference.txt'), 'w') as f_reference:
